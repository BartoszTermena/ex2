...
@Directive({ selector: 'canvas[chart]', exportAs: 'base-chart' })
export class BaseChartDirective
  implements OnDestroy, OnChanges, OnInit, Colors {
  ...
  private getDatasets(): any {
    ...
    if (
      !this.datasets ||
      (!this.datasets.length && this.data && this.data.length)
    ) {
      if (Array.isArray(this.data[0])) {
        datasets = (this.data as Array<number[]>).map(
      ...
      datasets = (this.datasets || datasets).map(
        (elm: number, index: number) => {
          ...
          if (this.colors && this.colors.length) {
            ...
            Object.assign(
              newElm,
              getColors(this.chartType, index, newElm.data.length)
            );
          }
          return newElm;
        }
